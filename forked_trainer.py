...
class Trainer:
    ...
    def train(self, net, train_producer, test_producer, epochs=100,
        ...
        if not self.dryrun and log2file:
            print('Dumping output to:', self.path + '/log')
            if self.restore:
        ...
        for epoch in range(self.highscore_epoch, epochs+1):
            ...
            print('#### NEW LEARNING RATE %e ####' % learning_rate, file=log)
            if useAdam:
            ...
            if len(self.class_weight) == 3:
                print('Train acc:\n',
                      ...
                      tuple((acc_sum/i).tolist()), file=log)
            ...
            if test_acc[-1].item() > self.test_highscore:
                self.test_highscore = test_acc[-1].item()
            ...
            if len(self.class_weight) == 3:
                print('Test acc:\n',
                      'N: %.4f  A: %.4f  O: %.4f  mean: %.4f'%
                      tuple(test_acc.tolist()), file=log)
            elif len(self.class_weight) == 4:
                print('Test acc:\n',
                      'N: %.4f  A: %.4f  O: %.4f ~: %.4f  mean: %.4f'%
                      tuple(test_acc.tolist()), file=log)
